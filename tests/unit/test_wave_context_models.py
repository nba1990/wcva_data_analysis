...
from src.eda import demand_and_outlook, workforce_operations, profile_summary, volunteer_recruitment_analysis
...
def test_wavecontext_validation_enforces_pct_ranges() -> None:
    ...
    assert 0 <= WAVE1_CONTEXT.finance.headline.financial_position_deteriorated_due_to_rising_costs_pct <= 100
...
def test_build_wave_context_from_df_matches_eda_outputs(tiny_df) -> None:
    ...
    assert ctx.demand.headline.increasing_demand_for_services_pct == int(round(dem["demand_pct_increased"]))
    assert (
...
def test_wave_registry_and_trend_series_round_trip() -> None:
    ...
    assert point["value"] == WAVE1_CONTEXT.demand.headline.increasing_demand_for_services_pct
...
def test_compare_helpers_use_pct_point_change() -> None:
    ...
    latest.demand.headline.increasing_demand_for_services_pct = first.demand.headline.increasing_demand_for_services_pct + 5
