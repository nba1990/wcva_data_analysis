...
- Code style (linting, formatting, import order) is consistent and enforced.
- Type checking and dependency security are part of the pipeline.
- Packaging remains buildable and secrets scanning is automated.
...
We use a **single GitHub Actions workflow** (`.github/workflows/ci.yml`) with **pip** and **requirements-dev.txt** for CI tooling (plus `requirements.txt` for runtime). The workflow defines separate jobs:
...
- **lint**: `ruff check .`, `ruff format --check .`, and `lint-imports`. Ruff replaces the previous Black/isort split and keeps linting and formatting in one toolchain. Import-linter continues to enforce architectural boundaries.
...
- **security**: `pip-audit`, `bandit`, and `detect-secrets` for dependency, static security, and accidental secret exposure checks.
- **package**: `python -m build --sdist --wheel` to verify the repo still builds cleanly as a Python distribution.
- **docs**: diagram generation plus `sphinx-build -b html docs/source docs/build/html` so the published documentation is rebuilt on every push and pull request.
- **local runner**: `scripts/run_quality_checks.sh` provides a one-command way to run the same categories of checks locally; `--quick` keeps a faster day-to-day subset for development loops.
...
- Every push/PR gets consistent feedback: tests, lint/format, types, security, packaging, docs, and architecture contracts in one place.
- Contributors use the same tools locally (Ruff, pytest, mypy, import-linter, security checks) as CI; CONTRIBUTING.md and README describe both the individual commands and the `scripts/run_quality_checks.sh` wrapper.
...
- New contributors and future maintainers have a single workflow to read and one place to add new steps without navigating multiple legacy configs.
