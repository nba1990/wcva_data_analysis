...
from src.charts import donut_chart, horizontal_bar_ranked, stacked_bar_ordinal
...
def render_executive_insights_list_html(
    ...
    *,
    ...
    sorted_items = (
        sorted(items, key=lambda x: x.get(rank_key, 0)) if rank_key else list(items)
    )
...
def build_slides(df, palette_mode: str) -> list[dict]:
    ...
    registry = build_wave_registry_from_current_data(df)
    ...
    core_trends = (
        trend_df[trend_df["metric_id"].isin(core_metric_ids)].copy()
    ...
    if not core_trends.empty:
        ...
        trends_pivot = trends_df.pivot_table(
        ...
        # Header row
        cols = ["Wave"] + [
            c for c in trends_pivot.columns if c not in {"wave_number", "Wave"}
        ]
        ...
        wave_summary_html = (
            f"<p style='font-size:0.75em;color:#555'>{wave_summary_text}</p>"
        )
    ...
    fig_demand = stacked_bar_ordinal(
        dem["demand"],
        TITLE,
        n,
        height=180,
    ...
    fig_financial = stacked_bar_ordinal(
        dem["financial"],
        TITLE,
        n,
        height=180,
    ...
    fig_rec_barriers = horizontal_bar_ranked(
        ...
        "label",
        "count",
        "Top barriers to volunteer recruitment",
        n,
        max_items=6,
        height=300,
    )
    fig_ret_barriers = horizontal_bar_ranked(
        ...
        "label",
        "count",
        "Top barriers to volunteer retention",
        n,
        max_items=6,
        height=300,
    )
    ...
    infographic_data = [
        ...
        (
            "Financial position deteriorated (last 3 mth)",
            dem["financial_pct_deteriorated"],
        ),
    ...
    slides = [
        ...
        },
        {
            ...
            "body": (
                ...
                + (
                    f" <br><li>Among organisations whose financial position deteriorated (last 3 months), <strong>{cross['pct_rec_difficulty_if_finance_deteriorated']}%</strong> find recruitment difficult, vs <strong>{cross['pct_rec_difficulty_if_finance_not_deteriorated']}%</strong> where financial position has not deteriorated.</li>"
                    if cross
                    else ""
                )
                + "</ul>"
...
def _earned_settlement_body(vt: dict) -> str:
    ...
    agree = es[es["value"].isin(["Strongly agree", "Somewhat agree"])]["count"].sum()
    ...
    *,
    ...
    for s in slides:
        ...
        if include_chart_images and s["chart"] is not None:
            ...
            alt = s.get("alt_text", "")
            chart_html = (
                f'<img class="chart" src="data:image/png;base64,{b64}" alt="{alt}">'
            )
...
class BaromedrPDF(FPDF):
    ...
    def _render_exec_highlights(self, highlights: list[dict]):
        ...
        for h in sorted(highlights, key=lambda x: x.get("rank", 0)):
            ...
            self.multi_cell(
                self.w - self.l_margin - self.r_margin - 8, 5, _strip_html(h["detail"])
            )
